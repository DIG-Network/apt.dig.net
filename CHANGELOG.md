# Changelog

All notable changes to this project are documented here.
This project adheres to [Semantic Versioning](https://semver.org) and
[Conventional Commits](https://www.conventionalcommits.org).

## [0.5.0] - 2026-08-19

### Features

- **apt:** Re-ingest upstream releases on a schedule, and serve dig-dns + dig-app (#9)

## [0.4.3] - 2026-08-06

### Chores

- Add .gitattributes to pin LF line endings (#2198)

## [0.4.2] - 2026-07-19

### Chores

- **pkg:** Repoint dig-store asset/release source to DIG-Network/digs (#764) (#7)

## [0.4.1] - 2026-07-18

### Chores

- **apt:** Arch-audit #353 — single-source dig-node port, SPEC.md, runbooks, DRY tests (#6)

## [0.4.0] - 2026-07-17

### Features

- **packaging:** Cut over store CLI to the dig-store binary + asset name (#5)

## [0.3.2] - 2026-07-13

### Chores

- Fix dig-node port from 8080 to 9778 (#4)

## [0.3.1] - 2026-07-12

### Bug Fixes

- Correct Discord invite (imposter link -> official) (#3)

## [0.3.0] - 2026-07-12

### Features

- **packaging:** Ship the digs alias binary in the digstore .deb (#2)

## [0.2.0] - 2026-07-10

### Features

- **frontend-baseline:** Embed bug-report widget + expose build version (#1)

## [0.1.0] - 2026-07-04

### Build

- Add package.json version for tag-driven release pipeline (#230)

### CI

- Enforce version increment in PRs (package.json / Cargo.toml)
- Enforce Conventional Commits with commitlint on PRs
- Enforce Conventional Commits with commitlint on PRs
- Release automation — git-cliff changelog + tag on merge, deploy on tag (#230 Unit 2)

### Chores

- **changelog:** Add git-cliff config for Conventional-Commit changelog


